...
export async function main() {
  ...
  let indexOfCycle = 0,
    j = 0;
  let indexOfPassedExecs = 0,
    indexOfFailedExecs = 0,
    indexOfPendingExecs = 0,
  ...
  let allow_duplicity_of_cycles: boolean =
    configZephyr.zephyrDefaultOptions.skip_duplicityCycle_verify;
  ...
  while (indexOfCycle < unique.length) {
    ...
    await datas
      .getCycleId(
        ...
        cycle_proccess_argv,
        allow_duplicity_of_cycles,
        current_used_cycle_id
      )
      .then(async function (cycleId: any) {
          ...
          await datas
            ...
            .then(async function ([response, current_used_cycle]: string) {
              ...
              if (wip == true && count_pending_its != index.length) {
                ...
                count_pending_its == index.length &&
                passed == false &&
                count_failed_its == 0
              ) {
